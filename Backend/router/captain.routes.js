...
router.post('/register',[
    ...
    body('vehicle.vehicleType').isIn(["car","bike","auto","truck"]).withMessage('Invalid')
],
captainController.registerCaptain
)
router.post('/login',[
    body('email').isEmail().withMessage('Invalid Email'),
    body('password').isLength({min:6}).withMessage('Password must be at least 6 charcter long')
],
